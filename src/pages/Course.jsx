...
const Course = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const courseRes = await axios.get(`https://wisdomize-backend.onrender.com/api/courses/${courseid}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        // Fetch enrollment status
        const enrollmentRes = await axios.get(
          `https://wisdomize-backend.onrender.com/api/enrollment/check/${user?.email}/${courseid}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );

        // Fetch reviews
        const reviewsRes = await axios.get(
          `https://wisdomize-backend.onrender.com/api/reviews/${courseid}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleEnrollment = async () => {
    ...
    try {
      await axios.post(
        "https://wisdomize-backend.onrender.com/api/enrollment",
