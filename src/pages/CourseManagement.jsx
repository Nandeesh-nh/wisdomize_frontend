...
const CourseManagement = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const response = await axios.get(`https://wisdomize-backend.onrender.com/api/courses/instructor/${user.email}`, {
          headers: {
  ...
  const deleteCourse = async (courseId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://wisdomize-backend.onrender.com/api/courses/${courseId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
